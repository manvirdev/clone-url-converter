function convert(str: string): string {
  ...
  function testGitHubHttpsCloneLink(input: string): boolean {
  ...
  function convertToHttps(str: string): string {
    ...
    return str.replace(/^git@github\.com:/, 'https://github.com/').replace(/\.git$/, '');
  }
  
  function convertToSSH(str: string): string {
    ...
    return str.replace(/^https:\/\/github.com\//, 'git@github.com:').replace(/\.git$/, '');
  }
